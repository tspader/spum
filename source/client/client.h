...
typedef struct {
    dn_dynamic_array(dn_string_t) messages;
} sp_client_log_t;
...
void         sp_client_load_match(sp_match_data_t* match);
...
void sp_client_init() {
    ...
    sp_client_load_match(&sp_sample_match);
}
...
void sp_client_update() {
    ...
    if (nk_begin(nk, "spum", nk_rect(10, 10, 400, 800), NK_WINDOW_BORDER | NK_WINDOW_MOVABLE | NK_WINDOW_CLOSABLE)) {
        if (nk_tree_push(nk, NK_TREE_TAB, "Client Information", NULL)) {
...
void sp_client_draw_match() {
    ...
    if (nk_begin(nk, dn_string_builder_write_cstr(&builder), nk_rect(420, 10, 1200, 1200), NK_WINDOW_TITLE | NK_WINDOW_SCALABLE | NK_WINDOW_MOVABLE)) {
        f32 play_height = 600 + (nk->style.window.padding.y * 5);
        f32 panel_height = 150;
        dn_vector2_t card_size = { 
            .x = 50 - (nk->style.window.padding.x * 4), 
            .y = panel_height - (nk->style.window.padding.y * 2) 
        };

        nk_layout_row(nk, NK_DYNAMIC, play_height, 3, dn_arr_lval(f32, .25, .5, .25));
        ...
        if (nk_group_begin(nk, "sp_group:play", NK_WINDOW_NO_SCROLLBAR)) {
            ...
            if (nk_group_begin(nk, "sp_group:play:opponent_bench", NK_WINDOW_NO_SCROLLBAR)) {
                nk_layout_row_dynamic(nk, card_size.y, 3);
                nk_button_label(nk, "Bench 1");
                nk_button_label(nk, "Bench 2");
                nk_button_label(nk, "Bench 3");
                nk_group_end(nk);
            }

            if (nk_group_begin(nk, "sp_group:play:opponent_active", NK_WINDOW_NO_SCROLLBAR)) {
                nk_layout_row_dynamic(nk, card_size.y, 1);

                dn_string_t label = sp_player_is_active_slot_occupied(player) ? 
                    sp_find_card_name(player->active.card) :
                    dn_string_literal("Active");

                nk_button_dn_string(nk, label);
                nk_group_end(nk);
            }

            if (nk_group_begin(nk, "sp_group:play:active", NK_WINDOW_NO_SCROLLBAR)) {
                nk_layout_row_dynamic(nk, card_size.y, 1);
                nk_button_label(nk, "Active");
                nk_group_end(nk);
            }
        ...
        if (nk_group_begin(nk, "sp_group:actions", NK_WINDOW_BORDER)) {
            ...
            dn_dynamic_array(sp_match_action_t) actions = sp_match_calc_actions(&sp_client.match, sp_client.ui.selected_card);
            ...
            dn_for(index, actions.size) {
                ...
                if (nk_selectable_dn_string(nk, sp_match_action_to_string(action), NK_TEXT_LEFT, &selected)) {
            ...
            if (nk_button_dn_string(nk, dn_string_literal("Confirm"))) {
            // if (nk_button_dn_string_styled(nk, &confirm, dn_string_literal("Confirm"))) {
        ...
        if (nk_group_begin(nk, "sp_group:log", NK_WINDOW_BORDER)) {
            ...
            dn_for(index, sp_client.log.messages.size) {
                dn_string_t* message = (dn_string_t*)dn_dynamic_array_at(&sp_client.log.messages, index);
                nk_dn_string_wrap(nk, *message);
            }
...
void sp_client_process_match_event(sp_net_match_event_t* event) {    
    switch(event->kind) {
        case SP_NET_MATCH_EVENT_KIND_BEGIN: {
            ...
            sp_client.usernames.opponent = dn_string_copy(dn_str_buffer_view(event->username), &dn_allocators.standard.allocator);
            break;
        }
        case SP_NET_MATCH_EVENT_KIND_SYNC: {
            DN_LOG("SP_NET_MATCH_EVENT_KIND_SYNC");
            sp_client.match_search.ready = true;
            sp_client_load_match(&event->state);
            break;
        }
...
void sp_client_process_match_action_event(sp_net_match_action_event_t* event) {
    ...
    switch (result->kind) {
        ...
        case SP_MATCH_ACTION_RESULT_NOT_YOUR_TURN: {
            sp_client_log_push(&sp_client.log, sp_match_action_result_to_string(&event->action, &event->result));
            break;
        }
        default: {
            DN_UNREACHABLE();
...
void sp_client_load_match(sp_match_data_t* match) {
    sp_client.match = *match;

    dn_dynamic_array_clear(&sp_client.log.messages);
...
sp_player_t* sp_client_get_player() {
    return &sp_client.match.players[0];
}

sp_player_t* sp_client_get_opponent() {
    return &sp_client.match.players[1];
}
...
void sp_client_log_init(sp_client_log_t* log) {
    dn_dynamic_array_init(&log->messages, sizeof(dn_string_t), &dn_allocators.standard.allocator);
}

void sp_client_log_push(sp_client_log_t* log, dn_string_t str) {
    dn_string_t copy = dn_string_copy(str, log->messages.allocator);
    dn_dynamic_array_push(&log->messages, &copy);
}

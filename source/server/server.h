...
void sp_server_process_request(sp_session_t* session, sp_net_request_t* request) {
  ...
  switch(request->op) {
    ...
    case SP_OPCODE_MATCH_REQUEST: {
     ...
     } sp_searching_client_t;
     sp_searching_client_t clients [2] = dn_zero_initialize();
     ...
     dn_for_arr(clients, i) {
       sp_searching_client_t* client = &clients[i];
       sp_searching_client_t* other_client = &clients[(i + 1) % 2];
    ...
    case SP_OPCODE_CLIENT_MATCH_ACTION: {
      ...
      sp_match_action_result_t result = sp_match_process_action(match, player, &request->match_action.action);
      sp_net_padded_response_t response = {
        .payload = {
          .op = SP_OPCODE_MATCH_EVENT,
          .match_event = {
